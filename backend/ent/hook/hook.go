...
// The RequestLogPayloadFunc type is an adapter to allow the use of ordinary
// function as RequestLogPayload mutator.
type RequestLogPayloadFunc func(context.Context, *ent.RequestLogPayloadMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RequestLogPayloadFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RequestLogPayloadMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RequestLogPayloadMutation", m)
}
