...
func TestHandleStreamingResponse_CapturesTranscriptAsResponseBody(t *testing.T) {
	gin.SetMode(gin.TestMode)
	svc := newMinimalGatewayService()
	...
	require.Contains(t, string(result.responseBody), "content_block_delta")
	require.Contains(t, string(result.responseBody), "hello world")
}
