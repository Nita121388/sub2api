<template>
  <div v-if="selectedIds.length > 0" class="mb-4 flex items-center justify-between p-3 bg-primary-50 rounded-lg dark:bg-primary-900/20">
    ...
    <div class="flex gap-2">
      ...
      <button @click="$emit('batch-test')" :disabled="batchTesting" class="btn btn-secondary btn-sm disabled:opacity-60 disabled:cursor-not-allowed">
        {{ batchTesting ? t('admin.accounts.bulkActions.testing') : t('admin.accounts.bulkActions.batchTest') }}
      </button>
